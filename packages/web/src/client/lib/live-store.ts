...
const processEvent = (state: LiveSessionState, event: StoredEvent): LiveSessionState => {
...
const processLink = (state: LiveSessionState, link: LinkLike): LiveSessionState => {
	if (link.type === "spawn") {
...
const createLiveSessionStore = (sessionId: () => string | undefined) => {
	...
	createEffect(() => {
		...
		const fetchEventPages = async (
			...
			const res = await fetch(`/api/sessions/${id}/events?offset=${offset}&limit=${PAGE_LIMIT}`, {
				headers,
			});
		...
		(async () => {
			try {
				...
				const headers: Readonly<Record<string, string>> = token
	...
	createEffect(() => {
		...
		const validLinks = links.filter((l): l is LinkLike => typeof l === "object" && l !== null);
