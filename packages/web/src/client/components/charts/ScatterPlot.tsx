...
export const ScatterPlot = <T,>(props: ScatterPlotProps<T>): ReturnType<Component> => {
	...
	const sorted = createMemo(() =>
		[...props.data].sort((a, b) => props.x(a).localeCompare(props.x(b))),
	);
						...
						<For each={sorted()}>
							{(d, i) => {
								...
								return (
									<circle
										role="menuitem"
										aria-label={props.tooltipLabel?.(d) ?? `${props.x(d)}: ${fmtY()(props.y(d))}`}
										cx={px()}
										cy={py()}
										...
										onMouseEnter={(e) => {
											...
											const label =
												props.tooltipLabel?.(d) ?? `${props.x(d)}: ${fmtY()(props.y(d))}`;
				...
				<div class="mt-2 flex gap-3 px-2 text-xs text-muted">
					<div class="flex items-center gap-1">
						<span
							...
							style={{ "background-color": DRIFT_COLORS.good }}
						/>
					...
					<div class="flex items-center gap-1">
						<span
							class="inline-block h-2.5 w-2.5 rounded-[2px]"
							style={{ "background-color": DRIFT_COLORS.warn }}
						/>
					...
					<div class="flex items-center gap-1">
						<span
							class="inline-block h-2.5 w-2.5 rounded-[2px]"
							style={{ "background-color": DRIFT_COLORS.bad }}
						/>
