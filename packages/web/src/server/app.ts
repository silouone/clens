...
import { authToken, cors, validateSessionId } from "./middleware/security";
...
import { createGlobalSessionsRoute, createSessionsRoute } from "./routes/sessions";
...
const createApp = (options: AppOptions) => {
	...
	const api = app
		.get("/api/health", (c) => c.json({ status: "ok" as const, ts: Date.now() }))
		.get("/api/projects", (c) => c.json({ data: projects }))
