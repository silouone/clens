...
const groupByDate = (
	rows: readonly AnalyticsSummaryRow[],
): ReadonlyMap<string, readonly AnalyticsSummaryRow[]> => {
...
const computeDailyUsage = (
	byDate: ReadonlyMap<string, readonly AnalyticsSummaryRow[]>,
): readonly DailyUsageMetrics[] =>
	[...byDate.entries()]
		...
		.map(([date, rows]) => {
			...
			return {
				...
				avg_duration_ms:
					rows.length > 0 ? rows.reduce((s, r) => s + r.duration_ms, 0) / rows.length : 0,
				median_duration_ms: median(rows.map((r) => r.duration_ms)),
				avg_agent_count:
					rows.length > 0 ? rows.reduce((s, r) => s + r.agent_count, 0) / rows.length : 0,
...
const modelDisplayLabel = (model: string | null | undefined): string | null => {
	...
	if (base === "" || lower === "<synthetic>" || lower === "synthetic" || lower === "unknown")
		return null;
...
const computeModelBreakdown = (rows: readonly AnalyticsSummaryRow[]): readonly ModelBreakdown[] => {
	const byModel = new Map<
		string,
		{ count: number; cost: number; tokens: number; duration: number }
	>();
	rows.forEach((r) => {
...
const computeAgentTypeBreakdown = (
	rows: readonly AnalyticsSummaryRow[],
): readonly AgentTypeBreakdown[] => {
...
const computeUsageMetrics = (
	...
	const { current, previous } = window
		? splitByCustomWindow(rows, window)
...
const computeDailyInsights = (
	byDate: ReadonlyMap<string, readonly AnalyticsSummaryRow[]>,
): readonly DailyInsightsMetrics[] =>
	[...byDate.entries()]
		.sort(([a], [b]) => a.localeCompare(b))
		.map(([date, rows]) => {
			const totalToolCalls = rows.reduce((s, r) => s + r.tool_call_count, 0);
			...
			const totalChainLinks = rows.reduce(
				(s, r) => s + (r.edit_chain_links ?? r.edit_chain_count),
				0,
			);
...
const computeInsightsTotals = (rows: readonly AnalyticsSummaryRow[]): InsightsTotals => {
	if (rows.length === 0) {
		return {
			sessions: 0,
			backtrack_rate: 0,
			abandoned_edit_rate: 0,
			avg_drift_score: Number.NaN,
	...
	const totalReasoning = rows.reduce(
		(s, r) => s + Object.values(r.reasoning_by_intent).reduce((a, b) => a + b, 0),
		0,
	);
	const totalToolCalls = rows.reduce((s, r) => s + r.tool_call_count, 0);
...
const computeInsightsMetrics = (
	...
	const planDriftPoints: readonly PlanDriftPoint[] = current.flatMap(
		(r): readonly PlanDriftPoint[] =>
...
const windowFromQuery = (c: {
	req: { query: (k: string) => string | undefined };
}): CustomWindow | undefined => parseCustomWindow(c.req.query("from"), c.req.query("to"));
...
export const createAnalyticsRoute = (projectDir: string) => {
	...
	app.get("/usage", (c) => {
		...
		log.info(
			`GET /api/analytics/usage range=${range}${window ? ` window=${window.from}..${window.to}` : ""}`,
		);
		const rows = loadRows(projectDir);
		const data = computeUsageMetrics(
			rows,
			range,
			loadRawStartTimes(projectDir),
			loadPlan(projectDir),
			window,
		);
		return c.json({ data });
	...
	app.get("/insights", (c) => {
		...
		const window = windowFromQuery(c);
		log.info(
			`GET /api/analytics/insights range=${range}${window ? ` window=${window.from}..${window.to}` : ""}`,
		);
		const rows = loadRows(projectDir);
...
export const createGlobalAnalyticsRoute = (
	...
	const effectiveDirsFor = (projectFilter?: string): readonly string[] => {
		...
		if (projectFilter) {
			return projects
				.filter((p) => p.id === projectFilter)
				.flatMap((p) => findClensCaptureDirs(p.path));
		}
	...
	app.get("/usage", (c) => {
		...
		const window = windowFromQuery(c);
		log.info(
			`GET /api/analytics/usage range=${range} project=${project ?? "all"}${window ? ` window=${window.from}..${window.to}` : ""}`,
		);
		const rows = loadAllRows(project);
		const data = computeUsageMetrics(
			rows,
			range,
			loadAllRawStartTimes(project),
			planFor(project),
			window,
		);
		return c.json({ data });
	...
	app.get("/insights", (c) => {
		...
		const window = windowFromQuery(c);
		log.info(
			`GET /api/analytics/insights range=${range} project=${project ?? "all"}${window ? ` window=${window.from}..${window.to}` : ""}`,
		);
		const rows = loadAllRows(project);
	...
	app.post("/rebuild", (c) => {
		...
		log.info(
			`Rebuilt analytics summary: ${total} sessions across ${effectiveDirs.length} capture dirs`,
		);
