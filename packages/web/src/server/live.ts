...
const startLiveWatcher = (projectDir: string): LiveWatcherHandle => {
	...
	const refs: {
		...
		pollTimers: readonly ReturnType<typeof setInterval>[];
	} = {
		watchers: [],
		pollTimers: [],
	};
				...
				.forEach((file) => {
					...
					newLines.forEach((line) => {
						try {
							...
							log.warn(
								`Malformed JSONL line in ${sessionId.slice(0, 8)}:`,
	...
	if (usePoll) {
		...
		try {
			refs.watchers = [
				...refs.watchers,
				watch(sessionsDir, { recursive: false }, handleSessionChange),
			];
			log.info(`Watching sessions: ${sessionsDir}`);
		} catch (err) {
			log.warn(
				`fs.watch failed for sessions, falling back to polling:`,
				err instanceof Error ? err.message : String(err),
			);
		...
		try {
			refs.watchers = [
				...refs.watchers,
				watch(distilledDir, { recursive: false }, handleDistillChange),
			];
			log.info(`Watching distilled: ${distilledDir}`);
		} catch (err) {
			log.warn(
				`fs.watch failed for distilled, falling back to polling:`,
				err instanceof Error ? err.message : String(err),
			);
	...
	return {
		stop: () => {
			refs.watchers.forEach((w) => {
				w.close();
			});
			refs.pollTimers.forEach((t) => {
				clearInterval(t);
			});
			refs.watchers = [];
			refs.pollTimers = [];
