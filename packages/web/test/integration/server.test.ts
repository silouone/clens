...
describe("Integration: Full server lifecycle", () => {
	...
	beforeAll(() => {
		...
		writeFileSync(
			...
			JSON.stringify({
				...
				user_messages: [
					{ t: 1000, content: "Fix the bug", message_type: "prompt", is_tool_result: false },
				],
	...
	test("GET /api/sessions/:id does NOT report tier_stale when the distill's tier matches the explicit config tier", async () => {
		...
		writeFileSync(
			`${TEST_DIR}/.clens/config.json`,
			JSON.stringify({ capture: true, pricing: "api" }),
		);
