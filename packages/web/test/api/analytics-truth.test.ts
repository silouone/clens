...
const noonDaysAgo = (daysAgo: number): number => {
	const now = new Date()
	return new Date(now.getFullYear(), now.getMonth(), now.getDate() - daysAgo, 12, 0, 0, 0).getTime()
}
...
const writeSession = (id: string, opts: DistillOpts): void => {
	writeFileSync(
		...
		[
			JSON.stringify({ event: "SessionStart", t: opts.startMs, sid: id, data: { source: "cli" }, context: {} }),
			JSON.stringify({ event: "Stop", t: opts.startMs + 1000, sid: id, data: { reason: "done" }, context: {} }),
		].join("\n") + "\n",
	)
	writeFileSync(
...
describe("analytics truth — paid/value/roi + window + cache guard", () => {
	...
	beforeAll(() => {
		...
		writeFileSync(`${TEST_DIR}/.clens/config.json`, JSON.stringify({ capture: true, plan: "max20x" }))

		// Two sessions in the current custom window, one in the previous span.
		writeSession(CUR_A, { startMs: noonDaysAgo(2), costUsd: 100, inputTokens: 1000, cacheReadTokens: 3000 })
		writeSession(CUR_B, { startMs: noonDaysAgo(1), costUsd: 50, inputTokens: 0, cacheReadTokens: 5000 })
		writeSession(PREV, { startMs: noonDaysAgo(9), costUsd: 999, inputTokens: 10, cacheReadTokens: 10 })
