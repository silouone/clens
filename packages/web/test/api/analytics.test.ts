...
const writeRawSession = (id: string, startMs: number): void => {
	const lines = [
		JSON.stringify({ event: "SessionStart", t: startMs, sid: id, data: { source: "cli" }, context: {} }),
		JSON.stringify({ event: "Stop", t: startMs + 1000, sid: id, data: { reason: "done" }, context: {} }),
	]
	...
	writeFileSync(
		...
		JSON.stringify({
			...
			stats: {
				...
				cost_estimate: opts.costUsd !== undefined
					? {
...
describe("analytics API", () => {
	...
	test("B19: avg_edit_chain_length is the real mean (links per chain), not chains per session", async () => {
		...
		const totalAvg =
			data.daily.reduce((acc: number, d: { avg_edit_chain_length: number }) => Math.max(acc, d.avg_edit_chain_length), 0)
	...
	test("B18: 7d window is exactly 7 local days (boundary session at day -6 is in, day -7 is out)", async () => {
		...
		const writeBoth = (id: string, daysAgo: number) => {
			...
			writeFileSync(
				...
				[
					JSON.stringify({ event: "SessionStart", t: ms, sid: id, data: { source: "cli" }, context: {} }),
					JSON.stringify({ event: "Stop", t: ms + 1000, sid: id, data: { reason: "done" }, context: {} }),
				].join("\n") + "\n",
			)
			writeFileSync(
				...
				JSON.stringify({
					session_id: id,
					start_time: ms,
					stats: { total_events: 1, duration_ms: 1, events_by_type: {}, tools_by_name: {}, tool_call_count: 0, failure_count: 0, failure_rate: 0, unique_files: [] },
