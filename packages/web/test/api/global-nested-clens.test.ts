...
describe("web API — global mode with nested .clens capture dirs", () => {
	...
	beforeAll(() => {
		...
		projects = [
			{ id: "monorepo", path: gitRoot, name: "monorepo", added_at: Date.now() },
		]
	...
	test("POST distill still 404s for an unknown session", async () => {
		const res = await app.request("/api/commands/sessions/99999999-9999-9999-9999-999999999999/distill", {
			method: "POST",
		})
		expect(res.status).toBe(404)
	})
...
describe("web API — global analytics with nested .clens capture dirs", () => {
	...
	const writeDistilled = (captureDir: string, sid: string, startMs: number, costUsd: number): void => {
		mkdirSync(join(captureDir, ".clens", "distilled"), { recursive: true })
		writeFileSync(
