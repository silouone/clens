...
describe("web API — global sessions", () => {
	...
	describe("GET /api/work-units (global)", () => {
		test("returns aggregated units array", async () => {
		...
		test("returns units tagged with project info when index exists", async () => {
			const index = {
				version: 1,
				updated_at: Date.now(),
				units: [
			...
			const alphaUnits = body.data.filter(
				(u: Record<string, unknown>) => u.project_id === "project-alpha",
			)
			expect(alphaUnits.length).toBe(1)
			expect(alphaUnits[0].id).toBe("wu-alpha-1")
