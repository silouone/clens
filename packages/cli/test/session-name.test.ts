...
describe("readSessionName", () => {
	...
	test("handles &amp; HTML entities in value", () => {
		...
		writeFileSync(
			...
			JSON.stringify({
				type: "custom-title",
				customTitle: '"EDITS GIT DIFF &amp; agent attribution"',
				sessionId: "abc123",
			}),
		);
	...
	test("works with real transcript format (mixed event types)", () => {
		...
		writeFileSync(
			...
			[
				...
				JSON.stringify({
					type: "custom-title",
					customTitle: '"Feature Implementation"',
					sessionId: "2002ebf7-e611-4ac9-ad20-be9714f5e697",
				}),
				JSON.stringify({
					type: "assistant",
					uuid: "a2",
