...
const renderPhaseBoundaries = (phases: readonly PhaseBoundaryDecision[]): readonly string[] => {
	...
	return phases.map((p) => `  ${dim(`[${p.phase_index}]`)}  ${p.phase_name}  ${dim(fmtTime(p.t))}`);
};
...
const renderAgentSpawns = (spawns: readonly AgentSpawnDecision[]): readonly string[] => {
	...
	const spawnLines = spawns.map(
		(s) =>
			`  ${fmtTime(s.t)}  ${cyan(s.agent_name)} ${dim(`(${s.agent_type})`)}  ${dim(s.agent_id.slice(0, 8))}`,
	);

	return [
		...spawnLines,
		"",
		dim(`  Summary: ${spawns.length} agent${spawns.length !== 1 ? "s" : ""} spawned`),
	];
};
 ...
 */
const renderTaskDelegations = (
	delegations: readonly TaskDelegationDecision[],
): readonly string[] => {
	...
	return [
		...
		"",
		dim(
			`  Summary: ${delegations.length} delegation${delegations.length !== 1 ? "s" : ""} to ${uniqueAgents.length} agent${uniqueAgents.length !== 1 ? "s" : ""}`,
		),
	];
};
...
const renderTaskCompletions = (
	completions: readonly TaskCompletionDecision[],
): readonly string[] => {
...
const partitionDecisions = (
	...
	taskDelegations: decisions.filter(
		(d): d is TaskDelegationDecision => d.type === "task_delegation",
	),
	taskCompletions: decisions.filter(
		(d): d is TaskCompletionDecision => d.type === "task_completion",
	),
});
...
const renderTimeSection = (distilled: DistilledSession): readonly string[] => {
	const wallClock =
		distilled.summary?.key_metrics.duration_human ?? fmtDuration(distilled.stats.duration_ms);
...
export const renderDecisionsSummary = (distilled: DistilledSession): string => {
	...
	const { timingGaps, toolPivots, phaseBoundaries, agentSpawns, taskDelegations, taskCompletions } =
	...
	const agentSections: readonly string[] =
		agentSpawns.length > 0 || taskDelegations.length > 0 || taskCompletions.length > 0
			? [
					"",
